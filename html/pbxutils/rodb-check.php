...
$dbconn = pg_connect("host=" . $IP . " dbname=pbxs user=postgres ")
    or die('Could not connect: ' . pg_last_error());

echo "<pre>";

nowQueryThis("select pg_last_xlog_receive_location(),pg_last_xlog_replay_location()");
nowQueryThis("select count(*),mode from pg_locks group by mode");
nowQueryThis("select pg_is_in_recovery()");
