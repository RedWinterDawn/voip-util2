...
function loadDistribute ($clients, $mpls, $servers, $location, $max) {
	$len = sizeof($clients);
	...
	for ($i = 0; $i < $len; $i++) {
		...
		//If so, die. 
    if (!in_array($clients[$i]['domain'], $mpls)) {
      continue;
    }
		if (min($servers) >= $max) {
			echo $location, "<br>";
...
echo "<h2>Mass Exodus</h2>
	<p class='red'>Special Mode, do not use this tool</p>
...
if ($action == "submit")
{
	...
	foreach ($secondaries as $secondary) {
		...
		$serverLoadQuery = "SELECT ip, load FROM pbxstatus WHERE site_id = '$second' AND status = 'active' ORDER BY load ASC;";	
		...
		$distResults = loadDistribute($clients, $mpls, $serverLoad, $second, $maxLoad);
		...
		foreach ($clients as $client) {
			...
			if (!in_array($client['domain'], $mpls)) {
				echo "<p class='purple'>${client['domain']} staying in .$source. because customer is not MPLS! DATABASE NOT UPDATED.</p>";
				continue;
			}
