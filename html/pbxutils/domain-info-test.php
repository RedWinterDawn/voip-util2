...
while ($domainRow = pg_fetch_array($domainResult, null, PGSQL_ASSOC)) {
	...
	$eventQuery = "SELECT added, description from event, (SELECT event_id FROM event_domain WHERE domain_id='".$domainRow['id']."') as domain WHERE event.id = domain.event_id order by number desc limit 5;";
...
//		echo "<td>" . $typeRow[''] . "</td>";
	...
	if ($event)		
	{
		echo "<tr><td></td><td></td><td colspan=5 rowspan=13>
			<table>
