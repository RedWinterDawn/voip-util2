...
if ($action == $gobutton)
{
	if ($domain != "")
	{
		...
		if (($order != "") && ($customer != "") && ($location != "") && ($carrier != "") && ($lec != "") && ($lan != "") && ($wan != ""))
		{
			$action = "add";
		}
...
switch ($action)
{
	case "add":
		...
		$name = pg_fetch_row(pg_query($rodb, "SELECT * FROM resource_group WHERE domain='".$domain."'")) or die('Could not find domain: '.$domain. ' '.pg_last_error());
		pg_close($rodb);
