...
if ($action=="search")
{
	...
	{	
		...
		$eventQ = "SELECT added AT TIME ZONE 'UTC-6', id, description FROM event WHERE added BETWEEN (timestamp '".$search."' AT TIME ZONE 'America/Boise') AND ((timestamp '".$search."' + interval '1 day') AT TIME ZONE 'America/Boise') ORDER BY number desc;";
...
if ($action=="eventList" && isset($domainName))
{
	...
	$eventsDB = pg_connect("host=rodb dbname=events user=postgres ") or die('Could not connect to "events" database: ' . pg_last_error());
	$eventQ = "SELECT added AT TIME ZONE 'UTC-6', description from event, event_domain WHERE domain_id ='".$pbxArray[0]['id']."' AND event_id = id ORDER BY number DESC";
