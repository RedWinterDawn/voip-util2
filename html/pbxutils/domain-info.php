...
function eventTable($id)
{
	...
	$eventQuery = "SELECT added AT TIME ZONE 'UTC-6' as added, description from event, (SELECT event_id FROM event_domain WHERE domain_id='".$id."') as domain WHERE event.id = domain.event_id order by number desc limit 10;";
