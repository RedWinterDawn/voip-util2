...
query = "DELETE FROM domestic WHERE carrier_id = 4"
try:
...
#insert new rates
query = "SELECT lrn, inter, intra FROM onvoy_domestic WHERE validto is NULL"
...
for rate in rates:
    try:
        if (count == 0):
            query = "INSERT INTO domestic (carrier_id, prefix, inter, intra) VALUES (4, %s, %s, %s)" %(rate[0], rate[1], rate[2])
        else:
