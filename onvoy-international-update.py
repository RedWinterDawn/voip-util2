...
query = "DELETE FROM international WHERE carrier_id = 4"
try:
...
#insert new rates
query = "SELECT name, code, cost FROM onvoy_international WHERE validto is NULL"
...
for rate in rates:
    try:
        if (count == 0):
            query = "INSERT INTO international (carrier_id, location, dst_code, cost) VALUES (4, '%s', %s, %s)" %(rate[0], rate[1], rate[2])
        else:
            query = "%s, (4, '%s', %s, %s)" %(query, rate[0], rate[1], rate[2])
