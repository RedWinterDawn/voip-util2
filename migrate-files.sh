...
then
	...
	then
		rsync -rav --delete-after $nfs::v4/pbxs/$pbx/ $basedir/$2/pbxs/$pbx/ 1>/dev/null 2>&1
	else
	...
	if [[ -e $basedir/$old_location/pbxs/$pbx ]]
	then
		rsync -rav --delete-after $basedir/$old_location/pbxs/$pbx/ $nfs::v4/pbxs/$pbx/ 1>/dev/null 2>&1
	else
		mkdir $basedir/$2/pbxs/$pbx 1>/dev/null 2>&1
		mkdir $basedir/$2/pbxs/$pbx/holdmusic 1>/dev/null 2>&1
		mkdir $basedir/$2/pbxs/$pbx/soundclips 1>/dev/null 2>&1
