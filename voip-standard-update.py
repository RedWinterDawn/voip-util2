...
query = "DELETE FROM domestic WHERE carrier_id = 7"
try:
...
#insert new rates
query = "SELECT lrn, inter, intra FROM voip_standard WHERE validto is NULL"
...
for rate in rates:
    try:
        if (count == 0):
            query = "INSERT INTO domestic (carrier_id, prefix, inter, intra) VALUES (7, %s, %s, %s)" %(rate[0], rate[1], rate[2])
        else:
            query = "%s, (7, %s, %s, %s)" %(query, rate[0], rate[1], rate[2])
