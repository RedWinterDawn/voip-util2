...
query = "DELETE FROM international WHERE carrier_id = 3"
try:
...
#insert new rates
query = "SELECT destination, fullcode, rate FROM level3_international WHERE validto is NULL"
...
for rate in rates:
    try:
        if (count == 0):
            query = "INSERT INTO international (carrier_id, location, dst_code, cost) VALUES (3, '%s', %s, %s)" %(rate[0], rate[1], rate[2])
        else:
            query = "%s, (3, '%s', %s, %s)" %(query, rate[0], rate[1], rate[2])
