...
query = "DELETE FROM domestic WHERE carrier_id = 2"
try:
...
#insert new rates
query = "SELECT substring(prefix::varchar(20) from 2) as newprefix, rate FROM iristel WHERE validto is NULL"
...
for rate in rates:
    try:
        if (count == 0):
            query = "INSERT INTO domestic (carrier_id, prefix, inter, intra) VALUES (2, %s, %s, %s)" %(rate[0], rate[1], rate[1])
        else:
            query = "%s, (5, %s, %s, %s)" %(query, rate[0], rate[1], rate[1])
